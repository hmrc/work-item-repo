...
case class EnsureIndexDelete(collection: String, index: String)
    extends Command[Int] {
  override def makeDocuments =
    BSONDocument("deleteIndexes" -> BSONString(collection),
  ...
  object ResultMaker extends BSONCommandResultMaker[Int] {
    def apply(document: BSONDocument) =
      CommandError
        ...
        .filterNot(
          _.getMessage().contains(s"index not found with name [$index]"))
        .toLeft(
          document.getAs[BSONInteger]("nIndexesWas").map(_.value.toInt).get)
  }
}
