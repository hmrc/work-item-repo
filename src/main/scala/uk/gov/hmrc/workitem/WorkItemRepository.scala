...
abstract class WorkItemRepository[T, ID](collectionName: String,
                                         ...
                                         config: Config)(
    implicit idFormat: Format[ID],
    mfItem: Manifest[T],
    mfID: Manifest[ID])
    extends ReactiveRepository[WorkItem[T], ID](collectionName,
                                                mongo,
                                                itemFormat,
                                                idFormat)
  ...
  override def metrics(
    ...
    Future
      .traverse(ProcessingStatus.processingStatuses.toList) { status =>
        count(status).map(value => s"$metricPrefix.${status.name}" -> value)
      }
      .map(_.toMap)
  }

  private implicit val dateFormats: Format[DateTime] =
  ...
  private def newWorkItem(receivedAt: DateTime,
                          availableAt: DateTime,
                          initialState: T => ProcessingStatus)(item: T) =
    WorkItem(
  ...
  override def indexes: Seq[Index] = Seq(
    Index(key = Seq(workItemFields.status -> IndexType.Ascending,
  ...
  def pushNew(item: T, receivedAt: DateTime)(
      implicit ec: ExecutionContext): Future[WorkItem[T]] =
    pushNew(item, receivedAt, toDo _)
  ...
  def pushNew(item: T, receivedAt: DateTime, availableAt: DateTime)(
      implicit ec: ExecutionContext): Future[WorkItem[T]] =
  ...
  def pushNew(items: Seq[T], receivedAt: DateTime)(
      implicit ec: ExecutionContext): Future[Seq[WorkItem[T]]] =
    pushNew(items, receivedAt, toDo _)

  def pushNew(items: Seq[T],
              receivedAt: DateTime,
              initialState: T => ProcessingStatus)(
      implicit ec: ExecutionContext): Future[Seq[WorkItem[T]]] =
  ...
  def pushNew(items: Seq[T],
      ...
      implicit ec: ExecutionContext): Future[Seq[WorkItem[T]]] = {
    val workItems =
      items.map(newWorkItem(receivedAt, availableAt, initialState))
  ...
  private implicit val read: Reads[IdList] = {
    implicit val objectIdReads: Reads[BSONObjectID] =
      ReactiveMongoFormats.objectIdRead
    Json.reads[IdList]
  }

  def pullOutstanding(failedBefore: DateTime, availableBefore: DateTime)(
      implicit ec: ExecutionContext): Future[Option[WorkItem[T]]] = {
  ...
  private def findNextItemId(failedBefore: DateTime, availableBefore: DateTime)(
      implicit ec: ExecutionContext): Future[Option[IdList]] = {

    def findNextItemIdByQuery(query: JsObject)(
        implicit ec: ExecutionContext): Future[Option[IdList]] =
      findAndUpdate(
        query = query,
    ...
    def todoQuery(failedBefore: DateTime,
                  availableBefore: DateTime): JsObject = {
      Json.obj(workItemFields.status -> ToDo,
               workItemFields.availableAt -> Json.obj("$lt" -> availableBefore))
    }

    def failedQuery(failedBefore: DateTime,
                    availableBefore: DateTime): JsObject = {
      Json.obj(
        "$or" -> Seq(
          Json.obj(
            ...
            workItemFields.availableAt -> Json.obj("$lt" -> availableBefore)),
          Json.obj(workItemFields.status -> Failed,
                   workItemFields.updatedAt -> Json.obj("$lt" -> failedBefore),
                   workItemFields.availableAt -> Json.obj("$exists" -> false))
        ))
    }

    def inProgressQuery(failedBefore: DateTime,
                        availableBefore: DateTime): JsObject = {
      Json.obj(workItemFields.status -> InProgress,
               workItemFields.updatedAt -> Json.obj(
                 "$lt" -> now.minus(inProgressRetryAfter)))
    }

    findNextItemIdByQuery(todoQuery(failedBefore, availableBefore)).flatMap {
      case None =>
        findNextItemIdByQuery(failedQuery(failedBefore, availableBefore))
          .flatMap {
            case None =>
              findNextItemIdByQuery(
                inProgressQuery(failedBefore, availableBefore))
  ...
  def markAs(id: ID,
             ...
             availableAt: Option[DateTime] = None)(
  ...
  def complete(id: ID, newStatus: ProcessingStatus with ResultStatus)(
      implicit ec: ExecutionContext): Future[Boolean] = {
    collection
      .update(
        selector = Json.obj(workItemFields.id -> id,
  ...
  def cancel(id: ID)(
      implicit ec: ExecutionContext): Future[StatusUpdateResult] = {
    import uk.gov.hmrc.workitem.StatusUpdateResult._
    findAndUpdate(
      query = Json.obj(
        workItemFields.id -> id,
        workItemFields.status -> Json.obj(
          "$in" -> List(
            ToDo,
            Failed,
            PermanentlyFailed,
            Ignored,
            Duplicate,
            Deferred)) // TODO we should be able to express the valid to/from states in traits of ProcessingStatus
      ),
      ...
      res.value match {
        case Some(item) =>
          Future.successful(
            Updated(
              previousStatus =
                Json.toJson(item).\(workItemFields.status).as[ProcessingStatus],
              newStatus = Cancelled
            ))
        case None =>
          findById(id).map {
  ...
  def count(state: ProcessingStatus)(
      implicit ec: ExecutionContext): Future[Int] =
    count(Json.obj(workItemFields.status -> state.name),
          ReadPreference.secondaryPreferred)

  private def setStatusOperation(newStatus: ProcessingStatus,
                                 availableAt: Option[DateTime]): JsObject = {
    val fields = Json.obj(
      workItemFields.status -> newStatus,
      workItemFields.updatedAt -> now
    ) ++ availableAt
      .map(when => Json.obj(workItemFields.availableAt -> when))
      .getOrElse(Json.obj())

    val statusUpdate = Json.obj("$set" -> fields)
    if (newStatus == Failed)
      statusUpdate ++ Json.obj(
        "$inc" -> Json.obj(workItemFields.failureCount -> 1))
    else statusUpdate
  }
...
object Operations {
  ...
  trait FindById[ID, T] {
    def findById(id: ID, readPreference: reactivemongo.api.ReadPreference)(
        implicit ec: ExecutionContext): Future[Option[WorkItem[T]]]
  }
}
