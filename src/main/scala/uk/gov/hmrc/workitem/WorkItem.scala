...
object WorkItem {
  ...
  implicit def workItemMongoFormat[T](
      implicit tFormat: Format[T]): Format[WorkItem[T]] = {
    import uk.gov.hmrc.mongo.json.ReactiveMongoFormats._
    mongoEntity(workItemFormat[T])
  }

  implicit def workItemRestFormat[T](
      implicit tFormat: Format[T]): Format[WorkItem[T]] = {
        ...
        json match {
          case JsString(s) =>
            Try {
              ...
              JsError(
                s"Could not parse $s as a DateTime with format ${dateTimeFormat.toString}")
            }
          case _ =>
    ...
    implicit val bsonIdFormat: Format[BSONObjectID] = Format(
      Reads.StringReads.map(stringObjectId =>
        BSONObjectID.parse(stringObjectId) match {
          case TrySuccess(bsonObjectID) => bsonObjectID
          case TryFailure(exception) =>
            throw new RuntimeException(
              s"'$stringObjectId' is not a valid BSONObjectID: $exception")
      }),
  ...
  def workItemFormat[T](implicit bsonIdFormat: Format[BSONObjectID],
    ...
    val reads = (
      (__ \ "id").read[BSONObjectID] and
        ...
        ((__ \ "availableAt").read[DateTime] or (__ \ "receivedAt")
          .read[DateTime]) and
